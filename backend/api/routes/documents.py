...
# Caps how many uploads can be "in flight" (bytes read but not yet extracted)
# at once, so N concurrent uploads can't buffer N * MAX_UPLOAD_BYTES in RAM
# simultaneously. Acquired before the file is read in upload_document(),
# released in _process_document()'s finally once extraction is done — the
# slot is held across the request/background-task boundary, not just within
# one function.
_UPLOAD_CONCURRENCY = asyncio.Semaphore(5)
...
async def upload_document(
    ...
    await _UPLOAD_CONCURRENCY.acquire()
    try:
